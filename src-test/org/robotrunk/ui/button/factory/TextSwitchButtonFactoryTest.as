package org.robotrunk.ui.button.factory {
	...
	public class TextSwitchButtonFactoryTest {
		private var factory:TextSwitchButtonFactory;
		...
		public function setUp():void {
			factory = new TextSwitchButtonFactory();
		}
		...
		public function createsButton():void {
			...
			mockStyleMap();
			mockDependencies();
			assertThat( factory.create( injector, params ), instanceOf( TextSwitchButtonImpl ) );
		}

		private function mockDependencies():void {
			mock( injector ).method( "getInstance" ).args( StyleMap, "testMap" ).returns( styleMap );
			mock( injector ).method( "getInstance" ).args( TextSwitchButtonImpl ).returns( new TextSwitchButtonImpl() );
