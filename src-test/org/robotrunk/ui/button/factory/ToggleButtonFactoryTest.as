package org.robotrunk.ui.button.factory {
	...
	public class ToggleButtonFactoryTest {
		private var factory:ToggleButtonFactory;
		...
		public function setUp():void {
			factory = new ToggleButtonFactory();
		}
		...
		public function createsButton():void {
			...
			mockStyleMap();
			mockInjections();
			assertThat( factory.create( injector, params ), instanceOf( ToggleButtonImpl ) );
		}

		private function mockInjections():void {
			mock( injector ).method( "getInstance" ).args( StyleMap, "testMap" ).returns( styleMap );
			mock( injector ).method( "getInstance" ).args( ToggleButtonImpl ).returns( new ToggleButtonImpl() );
