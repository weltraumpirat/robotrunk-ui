package org.robotrunk.ui.buttonbar {
	...
	public class SwitchButtonBarTest {
		private var buttons:Vector.<DisplayObject>;

		private var bar:SwitchButtonBarImpl;

		[Before]
		public function setUp():void {
			bar = new SwitchButtonBarImpl();
		...
		[Test]
		public function buttonIsSelectedAndDeselectedOnClick():void {
			buttons[3].dispatchEvent( new ButtonEvent( ButtonEvent.BUTTON_CLICK ) );
			assertTrue( SwitchButton( bar.buttons[3] ).active );
		...
		[Test]
		public function unclickedButtonsRemainUnaffected():void {
			buttons[3].dispatchEvent( new ButtonEvent( ButtonEvent.BUTTON_CLICK ) );
