package org.robotrunk.ui.buttonbar {
	...
	public class RelayButtonBarTest {
		private var buttons:Vector.<DisplayObject>;

		private var bar:RelayButtonBarImpl;

		[Before]
		public function setUp():void {
			bar = new RelayButtonBarImpl();
		...
		public function selectedButtonIsActive():void {
			...
			assertTrue( SwitchButton( bar.buttons[2] ).active );
		}

		[Test]
		public function unSelectedButtonsAreInactive():void {
			buttons[3].dispatchEvent( new ButtonEvent( ButtonEvent.BUTTON_CLICK ) );
			assertFalse( SwitchButton( bar.buttons[0] ).active );
